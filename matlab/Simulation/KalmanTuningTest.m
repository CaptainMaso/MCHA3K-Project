function cost = KalmanTuningTest(pw, pb, qw, qb, rt, rw, Theta, dTheta, ar, at, gy)
    ...
    for i = 2:size(Theta, 1)
        [Theta_k(i), dTheta_k(i), Bias_k(i), P_k] = IMUKalmanFilter(Theta_k(i-1), dTheta_k(i-1), Bias_k(i-1), P_k, gy(i - 1), ar(i-1), at(i-1), qw, qb, rw, rt);
    end
    
    cost = meansqr([Theta-Theta_k; dTheta-dTheta_k]);
end