...
#ifdef NO_LD_WRAP
void __wrap_cmd_parse(int, const char*[]) __asm__("_cmd_parse");
#endif

void __real_cmd_parse(int argc, const char* argv[]);

void __wrap_cmd_parse(int argc, const char* argv[])
{
    if (argc >= 1 && !strcmp(argv[0], "testcmd"))
    {
        printf(
            "\n"
            "This\n"
